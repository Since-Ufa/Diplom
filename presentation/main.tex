...
\begin{frame}{Введение. Металлические нанокластеры.}
	\begin{columns}
		...
		\vfill
		\justify
		Нанокластеры как таковые требуют стабилизации лигандами, в качестве которых используются органические соединения: аминокислоты, ДНК, олигонуклеотиды, тиолы и пр. 
...
\begin{frame}{Введение. Тиолы.}
	\justify
	\textbf{Тиолы и тиоляты} — органические соединения с функциональной тиольной/тиолятной группой (т.е. молекулы вида RSH/RS-, где R — органический радикал). К тиолам относятся множество биологически важных молекул:
	\begin{itemize}
...
\begin{frame}{Введение. Постановка задачи.}
	...
	\column{0.5\textwidth}
	    ...
	    \begin{alertblock}{Постановка задачи}
	    	\justify
	        \scriptsize В статье Xiaofang\ths Jia,et.al.\ths[1] получены экспериментальные спектры поглощения комплексов меди, кэпированных цистеином (Cys-CuNC). Исходя из приведённой масс-спектрометрии, наиболее часто встречается комплекс \ce{CuL2}. Тем не менее, основной вклад в спектры поглощения вносит по-видимому кластер \ce{Cu2L2}.
	    \end{alertblock}
...
\begin{frame}{\large Комплексы с метилтиогруппой. Спектр поглощения кластера \ce{[Cu2[SCH3]2]^{-2}}.}
	...
	\begin{columns}
		...
		\begin{figure}[!h]
			...
			\caption{\footnotesize Модельный спектр поглощения кластера \ce{[Cu2(SCH3)2]^{-2}}, полученные при помощи метода \textbf{M06-2X}/def2-TZVP в газовой фазе.\\ Гауссово уширение с полушириной 50\ths нм.}
		\end{figure}
		...
		\begin{figure}[!htb]
			...
			\caption{\footnotesize Основные вклады в спектр поглощения кластера \ce{[Cu2(SCH3)2]^{-2}} в газовой фазе.\\ Проекция изоповерхностей электронной плотности. Метод \textbf{M06-2X}/def2-TZVP.}
		\end{figure}
